...
.product-tab .merch-item-detail .item-image-info span i{
    vertical-align: middle;
    margin-right: 5px;
}

.product-tab .merch-item-detail .item-image-info span {
    color: black;
}

.product-tab .merch-item-detail .item-image-info .concert_name{
    font-size: 20px;
    font-weight: 600;
}
...
<div class="profile-container">
    ...
    <div class='tooltip'>
        ...
        <a href='history-ticket.php' class='sub-menu-bar concert_ticket active'><i class='material-icons'>confirmation_number</i>Ticket</a><br>
        <a href='#' class='sub-menu-bar merchandise_tab'><i class='material-icons'>local_mall</i>Merchandise</a>
    </div>
    ...
    <div class="history-container">
        ...
        ?>
        <form id='rating' method='POST' action='history-ticket.php'>
        <div class="all-merch-list">
        <?php
            if($type == 1)
            {
                $concert_info = mysqli_query($connect,"select A.Purchase_ID, A.Total_Price, E.Concert_Name, E.Concert_StartDate, E.Concert_Hor_Image, 
                F.Venue_Name, F.Venue_Location, G.Singer_Name from purchase A, customer B, s_ticket C, ticket_price D, concert E, venue F, singer G where A.Cust_ID = B.Cust_ID 
                and A.Purchase_ID = C.Purchase_ID and C.PriceID = D.Price_ID and D.Concert_ID = E.Concert_ID and E.Venue_ID = F.Venue_ID and D.Venue_ID = F.Venue_ID 
                and E.Singer_ID = G.Singer_ID and A.Purchase_ID = '$purchase_ID' group by A.Purchase_ID");
                $concert_info_run = mysqli_fetch_assoc($concert_info);

                $all_ticket_item = mysqli_query($connect,"select D.Price_Area, C.S_Ticket_Qty from purchase A, customer B, s_ticket C, ticket_price D, 
                concert E, venue F, singer G where A.Cust_ID = B.Cust_ID and A.Purchase_ID = C.Purchase_ID and C.PriceID = D.Price_ID and 
                D.Concert_ID = E.Concert_ID and E.Venue_ID = F.Venue_ID and D.Venue_ID = F.Venue_ID and E.Singer_ID = G.Singer_ID 
                and A.Purchase_ID = '$purchase_ID'");
            }
            else if($type == 2)
            {
                $concert_info = mysqli_query($connect,"select H.Rating_Star, H.Rating_Comment, H.Rating_Image, A.Purchase_ID, A.Total_Price, E.Concert_Name, 
                E.Concert_StartDate, E.Concert_Hor_Image, F.Venue_Name, F.Venue_Location, G.Singer_Name from purchase A, customer B, s_ticket C, ticket_price D, 
                concert E, venue F, singer G, rating H where A.Cust_ID = B.Cust_ID and H.Ticket_Purchase_ID = A.Purchase_ID and A.Purchase_ID = C.Purchase_ID 
                and C.PriceID = D.Price_ID and D.Concert_ID = E.Concert_ID and E.Venue_ID = F.Venue_ID and D.Venue_ID = F.Venue_ID and E.Singer_ID = G.Singer_ID 
                and A.Purchase_ID = '$purchase_ID' group by A.Purchase_ID");
                $concert_info_run = mysqli_fetch_assoc($concert_info);
            ...
            $count = 0;
            $img_count = 0; //how many img
            echo '<input type="text" name="purchase_id[]" value="'.$concert_info_run['Purchase_ID'].'"hidden readonly>';
            echo "<div class='merch-item-detail'>";
            echo '<div class="item-image-info"><img src="'.str_replace("../", "", $concert_info_run['Concert_Hor_Image']).'">';
            echo '<div style="float: left;">';
            echo "<span class='concert_name'>".$concert_info_run['Concert_Name'].'</span><br>';
            echo "<span class='time'><i class='material-icons'>event</i>".date_format(date_create($concert_info_run['Concert_StartDate']), "d M Y, H:i") . " GMT+8</span>".'<br>';
            echo "<span class='location'><i class='material-icons'>location_on</i>".$concert_info_run['Venue_Name'] . "</span><br>";
            echo "<span class='singer_name'><i class='material-icons'>audiotrack</i>".$concert_info_run['Singer_Name'].'</span><br>';
            echo "<span style='margin-left: 5px;'>";
            while($all_ticket_item_run = mysqli_fetch_assoc($all_ticket_item))
            {
                echo $all_ticket_item_run['Price_Area'].'x '.$all_ticket_item_run['S_Ticket_Qty'];
                if($count < mysqli_num_rows($all_ticket_item)-1)
                    echo ", ";
            ...
            {
                if($concert_info_run['Rating_Star']==1)
                {
                ...
                else if($concert_info_run['Rating_Star']==2)
                {
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star_border</span>';
                ...
                else if($concert_info_run['Rating_Star']==3)
                {
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                ...
                else if($concert_info_run['Rating_Star']==4)
                {
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                ...
                else if($concert_info_run['Rating_Star']==5)
                {
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                    echo '<span class="material-icons star-icon disBtn">star</span>';
                ...
                if($type == 1)
                {
                    echo '<div class="merch-rating-text"><label>Comment</label><textarea name="comment[]" class="merch-rating-input" rows="5" placeholder="What do you think of this concert?"></textarea>';
                }
                else if($type == 2)
                {
                    echo '<div class="merch-rating-text"><label>Comment</label><textarea name="comment[]" class="merch-rating-input" rows="5" disabled>'.$concert_info_run['Rating_Comment'].'</textarea>';
                }
                echo '</div>';
                ...
                if($type == 1)
                {
                    ...
                    echo '</div>';
                    echo '<label class="files-btn" for="files0'.$img_count.'"><span class="material-icons">photo_camera</span><span class="text">Upload Photo</span></label>';
                    echo '<input type="file" class="files-input" id="files0'.$img_count.'" name="files[]" hidden>';
                    echo '</div>';
                    $img_count++;
                }
                else if($type == 2)
                {
                    echo '<img class="preimg" src="'.str_replace("../", "", $concert_info_run['Rating_Image']).'" alt="" style="display:block">';
                    echo '</div>';
                    echo '</div>';
                    $img_count++;
                }
    ...
    $('#rating').on('submit',function(e){
        ...
        if(missing == 1)
        {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops',
                text: 'Please make sure this concert is being rated.',
        ...
        else
        {
            ...
            form_data.append("rating-concert",1);
            ...
            $.ajax({
                ...
                success: function(respone)
                {
                    ...
                    icon:'success',
                    didClose: () => 
                    window.open("history-ticket.php", "_self")});
                    console.log(respone);
                },
